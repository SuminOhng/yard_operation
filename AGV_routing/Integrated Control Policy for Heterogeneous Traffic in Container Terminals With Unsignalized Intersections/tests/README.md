...
Later milestones will add paper-network demand, a general 20-intersection TraCI runner, metrics, and multi-seed reproducibility gates. The smoke test is an adapter proof, and the paper-grid test proves only the declared reconstruction's internal consistency. Neither establishes numerical agreement with the paper.
