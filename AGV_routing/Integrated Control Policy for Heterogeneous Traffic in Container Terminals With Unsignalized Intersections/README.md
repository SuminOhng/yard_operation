...
The algorithm, first simulator-integration, paper-grid topology, seeded baseline-demand, and general 20-intersection runner milestones are implemented. The pure layer includes equations (1)-(17), HDV-aware phase extension (Algorithm 1), deterministic BP/VTR scheduling (Algorithm 2), distance-constrained IR-BP CAV routing (Algorithm 3), and a discrete one-cycle executor. A deterministic one-intersection smoke scenario connects those functions to live SUMO state through TraCI. Separate deterministic builders reconstruct the Fig. 6 topology as 20 intersections and 54 directed roads, then create exact-quota CAV/HDV demand with explicit, reviewable time-bin, OD, vehicle-class, speed, and initial-route assumptions. The general runner drives all 20 controllers, applies CAV and reconstructed seeded HDV routing per intersection encounter, and emits success or failure evidence.
...
The selected deterministic BP/VTR and IR-BP gates pass. The project environment is pinned to CPython 3.13.13 and SUMO/traci/sumolib 1.27.1 through `uv.lock`. The smoke gate discovers downstream candidates, applies and verifies one CAV route mutation, enforces mutually exclusive virtual phases, rejects collisions and teleports, and compares two fixed-seed decision traces. The static paper-grid and demand gates prove their declared topology, quotas, OD constraints, reachability, shortest initial routes, and provenance. The current seed-1 paper-fidelity live run does **not** drain: a diagnostic run reaches physical gridlock after 670 arrivals, with long unbounded HDV extensions. This is retained as a reproducible failure, not hidden with a safety cap and not claimed as numerical reproduction.
...
- [Full paper-grid TraCI runner](docs/10_paper_grid_runner.md)
...
Resolve the declared gridlock sensitivity decision before adding metrics: compare a separately labeled finite-extension safety profile and the unpublished cycle, geometry/capacity, and internal-origin demand assumptions. Never report a safety variant as the paper method. After a draining profile is justified, add metrics and numerical calibration.
