...
The algorithm and first simulator-integration milestones are complete. The pure layer includes equations (1)-(17), HDV-aware phase extension (Algorithm 1), deterministic BP/VTR scheduling (Algorithm 2), distance-constrained IR-BP CAV routing (Algorithm 3), and a discrete one-cycle executor. A deterministic one-intersection smoke scenario now connects those functions to live SUMO state through TraCI.
...
Reconstruct and validate the paper-scale four-by-five terminal network, then add seeded demand and experiment metrics without treating smoke assumptions as paper facts.
