...
- `sumo/demand/`: documentation plus deterministic generated baseline route and metadata files. The builder reads the experiment configuration and reconstructed network, enforces class-specific OD sets and exact quotas, and records dependency hashes.
...
Generated network and demand files must record the source files and command that created them. Never hand-edit generated `.net.xml`, `.rou.xml`, or metadata; use the corresponding builder.
